...
with st.sidebar:
    ...
    functionality = st.selectbox("Functionality", ["FindPrerequisites", "GetCoursesOffered", "CheckIfCompletedPrerequisites", 
    "EnrollInCourseOffering", "GetStudentEnrolledCourseOfferings", "DropFromCourseOffering"])
    if functionality == "FindPrerequisites":
...
if st.session_state.functionality == "FindPrerequisites":
    ...
    with st.form("search_form", clear_on_submit=False):
        ...
        with c1:
            student_id = st.text_input("Student ID", placeholder="e.g., 123", max_chars=20)
        with c2:
            subject_code = st.text_input("Subject Code", placeholder="e.g., MIST", max_chars=20)
        with c3:
            course_number = st.text_input("Course Number", placeholder="e.g., 460", max_chars=20)
    ...
    if submitted:
        service_url = f"{FASTAPI_BASE_URL}/{endpoint}"
        df = fetch_data(service_url, {"studentID": student_id, "courseOfferingID": course_offering_id})
        if df["EnrollmentSucceeded"].values[0] == 1:
            st.success("Enrollment successful.")
        else:
            st.error("Enrollment failed.")
            st.write(df["EnrollmentResponse"].values[0])
        #requests.get(service_url, {"studentID": student_id, "courseOfferingID": course_offering_id})
    ...
    if submitted:
        service_url = f"{FASTAPI_BASE_URL}/{endpoint}"
        # response = requests.get(service_url, {"studentID": student_id, "courseOfferingID": course_offering_id})
